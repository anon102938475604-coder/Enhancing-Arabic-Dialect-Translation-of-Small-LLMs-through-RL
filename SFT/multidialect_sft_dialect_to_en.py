...
OUTPUT_DIR = f"results/qwen2.5-7b-multidialect-dialect-to-EN"
...
# Control tokens: one per source dialect + a fixed target token (optional but fine to keep)
SRC_TOKENS = [f"<SRC:{d}>" for d in dialects]
SPECIAL_TOKENS = SRC_TOKENS
...
val_sources = val_df["ar"].astype(str).tolist()
...
test_refs    = test_df["en"].tolist()
test_srcs = test_df["ar"].tolist()
...
comet_res = comet.compute(
    predictions=all_preds,   # MT outputs (EN)
    references=test_refs,    # gold refs (EN)
    sources=test_srcs,       # sources (AR)
...
pd.DataFrame({
    ...
    "source_ar": test_df["ar"],
    "reference_en": test_refs,
    "prediction_en": all_preds
